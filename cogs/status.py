...
class StatusCog(commands.Cog):
    ...
    async def update_status(self):
        ...
        while not self.bot.is_closed():
            ...
            status_message = random.choice(self.config['status']['messages'])
            status_message = status_message.replace('{member_count}', str(member_count))
            status_message = status_message.replace('{bot_version}', self.config['bot_version'])

            status_type = self.config['status']['type']
            ...
            await asyncio.sleep(self.config['status']['interval'])
